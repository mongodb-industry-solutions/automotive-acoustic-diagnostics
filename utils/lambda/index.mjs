...
const getObjectIdHexString = (value) => {
  ...
  throw new Error("Vehicle ID is not a valid ObjectId.");
};
...
export const handler = async (event) => {
  ...
  try {
    ...
    const vehicleObjectId = ObjectId.createFromHexString(
      getObjectIdHexString(vehicleId),
    );
    const report = {
      inputTextTokenCount: parsed.usage?.input_tokens,
      results: [
        {
          tokenCount: parsed.usage?.output_tokens,
          outputText,
          completionReason: parsed.stop_reason,
        },
      ],
      modelId: parsed.model || MODEL_ID,
      vehicle_id: vehicleObjectId,
    };

    log("Parsed Bedrock response:", report);
    ...
    const insertResult = await collection.insertOne(report);
    log("Insert result:", insertResult);

    console.log(
      `Report inserted successfully for vehicle ${vehicleObjectId.toHexString()}`,
    );
    return {
      statusCode: 200,
